...
class WSInterfaceProtocol(WebSocketClientProtocol):
    ...
    def setUtterance(self, utt):
        ...
        self.summary[self.uttNumber] = {"hypothesis": "",
                                        "status": {"code": "", "reason": ""}}
        self.fileJson = self.dirOutput + "/" + str(self.uttFilename) + \
                        ".json.txt"
        try:
...
def run_transcribe(dirOutput, filename, username, password):
    # logging
    ...
    url = "wss://" + hostname + "/speech-to-text/api/v1/recognize?model=" \
            + args.model
    summary = {}
    factory = WSInterfaceFactory(q, summary, dirOutput, 'audio/wav',
                                 'en-US_BroadbandModel', url, headers, debug=False)
